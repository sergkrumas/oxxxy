...
### Версия 0.94 (дата не определена)
  ...
  - в ноябре начнётся лютый рефакторинг для поддержки zoom & panning, виджетов трансформации и мультиселекта
  - внедрение Panning & Zooming для холста в редакторе
      - ДЕТАЛИ
          - отрисовка всего за счёт вычислений на основе опорного ориджина и скейла холста
      ...
      - перещемение холста (panning) теперь работает через зажатое колесо мыши
      - зум холста (zomming) работает через колесо, **пока колесом нельзя менять параметры инструментов как раньше**
      - и зум и перемещение не работают пока адекватно, завтра придётся перепахивать весь код и часть даже удалить. Пока я всё не исправлю, невозможно будет пользоваться программой несколько дней
      - теперь рамка захвата адекватно отрисовывается при zoom и panning
      - восстановлено перемещение рамки захвата (функция доступна, когда не выбрано ни одного инструмента)
      - восстановлено задание курсоров для областей, образуемых касательными к прямоугольнику области захвата 
      - восстановлено автопозиционирование панели инструментов в зависимости от зума и смещения
      - восстановлено изменение границ рамки захвата
      - удалён код виджета трансформации
      - удалён код трансформации фона (фон будет теперь представлен картинкой, которую можно будет трансформировать как вздумается)
      - фоновое изображение (скриншот) теперь исполнено в виде пометки-изображения
          - заодно пришлось очень много рефакторить
      - восстановил корректную отрисовку дебажной превьюхи результата
      - восстановил корректную отрисовку списка элементов в дебажном режиме
      ...
      - вроде как восстановил лупу
      - зум при зажатом Alt работал только в одну строну, пришлось заменить Alt на комбинацию Shift+Ctrl
      - исправил позиционирование лупы при зуме и переносе когда ещё не задана область захвата. Вдобавок ушли баги *во время задания* области захвата 
      - автоматическое позиционирование панели инструментов теперь работает и при переносе холста (panning)
      - в контекстное мню добавлена галочка для отключения фона
      - подправил лупу, чтобы она выдавала нужное значение цвета при зуме и смещении холста
      - автоматическое позиционирование панели инструментов теперь работает и при зуме холста (zooming)
      - поправил задание прозрачности фона через TAB. **Когда фон можно будет свободно трансформировать и фон будет выходить за рамки экранов, надо будет просмотреть насколько адекватно всё отрисовывается и внести правки если нужно**
      - изменение управления: зумить можно теперь ещё при отображающиейся лупе с помощью одного колеса, а чтобы зумить лупу теперь надо зажимать ctrl и крутить колесо
      ...
      - при рефакторинге забыл задать смещение финальной отрисовки на координаты верхнего левого угла области захвата, из-за чего захватывалось не то, что нужно
      - в режиме дебага (`DEBUG=True`) больше не отображается оверлей результата. Посмотреть то, что в данный момент получается можно нажав на клавишу P (закрытие через эту же клавишу)
      - фикс краша: клавишу P не показывает результат, пока не задана область захвата
      - написал маппинг координат мыши на координаты холста
