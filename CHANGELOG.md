...
### Версия 0.94 (19 марта 2024): панорамирование и зум для редактора; возможность выделения нескольких пометок; перемещение, вращение и масштабирование пометок; обновлённая система истории действий
  ...
  - внедрение PZMT (Canvas Panning & Zooming и Elements Multiselect & Transformations) для холста в редакторе (продолжение)
      ...
      - инструмент «Текст»
          - при трансформациях текста начало стрелки перемещается так, чтобы быть ближе к любой точке на периметре геометрической оболочки текста, это относится к трансформациям самой стрелки. Так было и раньше, и в оригинальном Joxi, так что я просто возвращаю функнционал
      ...
      - UX: пометки «Текст» теперь можно переносить без проблем (раньше это было очень трудно сделать из-за срабатывания кода, который задавал курсор в поле ввода)
      ...
      - обновлённые пометки «Текст» теперь сохраняются в проектных файлах и загружаются из них
      ...
      - теперь при вращении пометки «Текст» буквы не будут колбасится на 1-2 пикселя в стороны благодаря опции «Pixmap-прокси для пометок типа «Текст»» в контекстном меню. По умолчанию она включена
...
### Версия 0.95 (дата не определена): UX и новые инструменты
  ...
  - исправлена отрисовка курсора в текстовом поле пометки «Текст» (актуально для всех строк кроме первой)
