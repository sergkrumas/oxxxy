...
### Версия 0.94 (дата не определена)
  ...
  - внедрение PZMT (Canvas Panning & Zooming и Elements Multiselect & Transformations) для холста в редакторе (продолжение)
      ...
      - рефакторинг метода создания и обновления фона скриншота
          ...
          - рефакторинг системы фильтрации: новый тип фильтрации. Теперь пометка может быть отмечена специальным флагом, который говорит о том, что она пройдёт фильтрацию, если прошёл фильтрацию элемент, в специальном атрибуте которого был индекс этого элемента
