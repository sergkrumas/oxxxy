...
### Версия 0.94 (дата не определена)
  ...
  - внедрение PZMT (Canvas Panning & Zooming и Elements Multiselect & Transformations) для холста в редакторе (продолжение)
      ...
      - рефакторинг метода создания и обновления фона скриншота
          ...
          - рефакторинг системы фильтрации: ввведён новый тип фильтрации. Теперь пометка может быть отмечена специальным флагом, который говорит о том, что она пройдёт фильтрацию, если прошёл фильтрацию элемент, в специальном атрибуте которого был индекс этого элемента
          - опция разбивки фона на несколько изображений скоро передет в контекстное меню
          - при рефакторинге системы фильтрации не учёл, что пометки, для которых эта фильтрация вводится, могут иметь модификации, и вот модифицированные версии уже могли бы обходить фильтрацию, если бы я сейчас не сделал поправку и не ввёл ещё один индекс, который уже будет копироваться (наследоваться) при модификациях
