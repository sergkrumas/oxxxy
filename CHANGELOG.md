...
### Версия 0.91 (26 декабря 2022)
  ...
  - Инструмент «Линия» теперь может рисовать ломанные линии при удерживании клавиши `Ctrl`, при этом каждый сегмент линии создаётся как отдельная пометка

### Версия 0.92 (12 мая 2023)
  - Ликвидирвоана зависимость от забагованного питоновского модуля `pynput`. Теперь есть надежда, что сочетания клавиш будут работать на линуксах
  ...
  - ~~Внедрён расширинный режим редактора для создания коллажей из готовых картинок и пометок поверх них~~
    ...
    - ~~через клавишу `P` можно смотреть на финальный результат в специальном вьювере, в котором можно приближаться к картинке и перемещать по ней в разные стороны. Это компенсирует то, что в самом редакторе невозможно приближаться и отдаляться. Контекстное меню содержит все требуемые команды для работы с просмоторщиком~~
  ...
  - Можно обрезать (кадрировать) пометки-изображения в специальном редакторе, вызываемом из контестного меню. Для этого надо сначала выделить нужную пометку-изображение через инструмент «Выделение и трансформации». *Сделано это для того, чтобы иметь возможность обрезать изображения, которые не помещаются на экране, но вообще можно обрезать любые. ((20 фев 26) предложение звучит очень таинственно, хотя, может, тогда ещё нельзя было зумить вьюпорт)* Область для кадрирования задаётся курсором мыши с зажатой левой кнопкой. Из редактора кадрирования можно выйти нажав клавишу `Esc`. В редакторе кадрирования переопределять область обрезки заново можно неограниченное количество раз. Затем, будучи удовлетворённым результатом, надо нажать на `Enter`. После такой обрезки всегда можно вернутся в редактор кадрирования и изменить такую образку сколько угодно раз. Сбросить обрезку можно через контекстное меню при условии, что пометка-изображение будет выделено инструментом «Выделение и трансформации».
...
### Версия 0.93 (1 октября 2023)
  ...
  - Во вьювере результата (доступен через клавишу P) можно задавать область захвата. После нажатия `Enter` область будет присвоена инструменту «Картинка» для вставки
  ...
  - Рамку во вьювере картинок теперь можно задавать и изменять с помощью виджета задания области захвата. Всё осталось работать через зажатый `Ctrl`.
  ...
  - Виджет трансформации теперь умеет в пропорциональное масштабирование, что как раз пригодится для картинок, и для активации достаточно зажать `Ctrl`. Для соблюдения равенства ширины и длины надо зажимать `Ctrl+Shift`
...
### Версия 0.94 (19 марта 2024): панорамирование и зум для редактора; возможность выделения нескольких пометок; перемещение, вращение и масштабирование пометок; обновлённая система истории действий
  ...
  - внедрение PZMT (Canvas Panning & Zooming и Elements Multiselect & Transformations) для холста в редакторе (продолжение)
      ...
      - опция «Сохранить скриншот в память» переименована в «Сохранить результат в память», команда «Достать все скриншоты из памяти» переименована в «Разложить на холсте все готовые изображения из памяти». Сделано из соображений того, что OXXXY теперь не просто редактор скриншотов, это ещё и специфический редактор изображений
      ...
      - активация инструмента «Выделение и Трансформация» обычно выделяла последнюю добавленную на данный момент пометку. Сейчас эта пометка - картинка, которая составляет фон, то есть непосредственно скриншот. На старте можно выделять только нефоновые пометки, поэтому я поправил выделение при активации инструмента так, чтобы учитывался фильтр выделения
  ...
  - UX: чекбоксы в контекстном меню редактора теперь выполнены в виде виджетов и клик по ним больше не прячет контекстное меню
  ...
  - внедрение PZMT (Canvas Panning & Zooming и Elements Multiselect & Transformations) для холста в редакторе (продолжение)
      ...
      - проанализировал и вспомнил детали обнуления истории действий в режиме «Сохранить результат в память». Каждый раз запускается новый объект класса редактора, поэтому история не переходит между сессиями этого режима
  ...
  - контекстное меню: поменял местами пункты меню, теперь они идут в удобном порядке
  ...
  - внедрение PZMT (Canvas Panning & Zooming и Elements Multiselect & Transformations) для холста в редакторе (продолжение)
      ...
      - **пояснение к картинке выше: на деле `diff` равен `α + 45`**, причём 45 это 45°, а не "условные 45"
      ...
      - поправил баг, из-за которого невозможно было сделать активной ни одну пометку (`self.active_element`)
      ...
      - восстановлена работа инструмента «Текст» с поддержкой нового выделения, собственных трансформаций и panning & zooming
          ...
          - при клике левой кнопкой мыши на активированную пометку «Текст» будет проставлен курсор в соотвествующее место для вставки текста
          - курсор можно перемещать клавишами `Влево` и `Вправо`
          - была идея, от которой отказался, оставлю её тут: каждая строка должна автоматически подстраивать свою ширину в зависимостио от близости рамки захвата, чтобы текст не выходил за эту рамку, и чтобы поддерживался случай с ненулевым углом поворота самой пометки. Так было раньше, но без поддержки угла поворота.
          - не получается пока сделать перенос курсора ввода клавишами `Вверх` и `Вниз` потому что я пока не знаю как извлекать номер строки из позиции курсора
          - текст можно наносить вместе с сопровождающей стрелкой как и раньше, для этого придётся зажать левую кнопку мыши. Текстовый блок появится в позиции курсора при отпускании кнопки мыши
          - если длина сопроводительной стрелки меньше 100 пикселей, то она автоматически удаляется
          - цвет подложки изменил с белого на градацию серого
      - инструмент «Лупа», улучшения
          - упростил код для отрисовки пометок инструмента
          - практически завершил опцию отображения в виде кружков
          - косметические улучшения: вся графика рисуется во фрейме второй пометки, чтобы толщины линий были одинаковыми
          - перестраховался от случая, когда отривовка не может быть выполнена по техническим причинам (например, когда скейл по одной из осей нулевой)
          - замечание: при непропорциональном масштабировании второй пометки касательные линии отрываются пот первой пометки. Чтобы этого избежать нужно производить только однородное масштабирование для второй пометки
          - обтравка кружком при выводе изображения во второй пометке
          - флажок «Линии» переименован в «Кружки»
          - пофиксил баг, возникший из-за невнимательности. Обтравка кружком включалась даже когда флаг был выключен
      - внёс в подсказку информацию о клавишах-модификаторах к инструменту «Выделение и Трансформации»
      - инструмент «Текст»
          - при трансформациях текста начало сопроводительной стрелки перемещается так, чтобы быть ближе к любой точке на периметре геометрической оболочки текста, это относится к трансформациям самой стрелки. Так было и раньше, и в оригинальном Joxi, так что я просто возвращаю функционал
          - текстовое поле теперь нормально реагирует на клавиши `Delete`, `Insert`, `Home`, `End`, `Page Up`, `Page Down`
      - недоработка: зависимые от фона пометки раньше обновлялись при трансформациях любых пометок, а не при трансформациях пометок-изображений формирующих фон
      - в контекстном меню теперь можно найти галочку «Антиальясинг и сглаживание пиксмапов». Выключение её позволяет добиться заметного повышения быстродействия, например, при сильном зуме или при переносе холста (panning). Эффект распросраняется только на работу в редакторе, на финальный результат галочка не влияет
      - поправил текстовые описания в окне настроек и цветовую гамму окна настроек
      - для второй подпометки инструмента «Лупа» теперь по дефолту включается пропорциональное масштабирование и зажимать клавишу `Shift` не придётся. Это сделано для того, чтобы касательные линии не отрывались от первой пометки-кружка
      - исправлен баг: код для текстовой пометки заблокировал движение курсора через клавиши `Влево` и `Вправо` при задании области захвата
      - исправлен баг: для вытянутых по одной из осей пометок (`Element.straight`) с предвращением иногда получался неудачный слишком узкий selection_box для целей выделения. Правильность поправки доказывается проведением восьми линий из импровизированного центра в стороны с шагом 45 градусов
      - исправлен баг: для вытянутых по одной из осей пометок (`Element.straight`) с предвращением неправильно работало вращение по шагу 45 градусов относительно холста
      - UX: пометки «Текст» теперь можно переносить по холсту без проблем (раньше это было очень трудно сделать из-за срабатывания кода, который включал режим редактирования текста)
      - исправлен баг: сам же его внёс при исправлении другого бага недавно. Из-за него стрелка, связанная с пометкой «Текст», неадекватно себя ведёт при переносе этого текста
      - рефакторинг системы модификаций: переменная `Element.source_index` переименована в `Element.source_indexes` и теперь является списком
      - исправил баги с инструментом «Копипейст», которые появились из-за недавних модификаций инструмента «Лупа»
      - при удалении группы (клавиша `Delete`) пометок теперь нужна всего одна невидимая removing-пометка
      - теперь при удалении взаимозависимой пометки инструментов «Лупа» и «Копипейст», удаляется и её спутник, если он не был выделен конечно
      - теперь при удалении пометки «Текст» зависимая стрелка тоже удаляется
      - исправил баг: из-за невнимательности запретил печатать знаки с зажатой клавишей `Shift` в поле пометки «Текст»
      - исправил бесячее поведение: бывает, что выделил пометку или несколько и начал тащить с зажатой кнопкой мыши по холсту, а курсор мыши в момент начала этой процедуры не находился над selection_box ни одной из выделенных пометок, и вроде бы тогда пометки не должны перемещаться, а они всё-таки перемещаются и это бесит. Исправил это наконец
      - рефакторинг метода создания и обновления фона скриншота
          - восстановлено создание фона при вызове редактора
          - команда «Нарисовать содержимое на фоне» больше не отменяет историю действий, но ещё пока не работает с ней так как подобает
          - рефакторинг системы фильтрации: ввведён дополнительный этап фильтрации. Теперь пометка может быть отмечена специальным флагом (`Element.pass_through_filter_only_if_allowed = True`), который говорит о том, что она пройдёт дополнительную фильтрацию, если первый этап фильтрации прошёл элемент, в специальном атрибуте `Element.allowed_indexes` которого включён `Element.pass2_unique_index` этого элемента. `Element.pass2_unique_index` равен `Element.unique_index` при создании пометки, но значение этого атрибута передаётся при копировании, чтобы при модификациях дополнительный этап фильтрации влиял и на модифицированные копии.
          - для чего нужен дополнительный этап фильтрации? Для того, чтобы можно было отменить результаты работы команд, которые работают с фоном. В историю действий надо положить слот с элементом типа `ToolID.removing`, чтобы убрать старый фон, но сам фон надо класть в первый слот, в котором обычно и лежат все картинки, которые формируют фон. Но если этот слот с элементом типа `ToolID.removing` не пройдёт фильтрацию (будет отменён через Ctrl+Z), то старые фоны появятся, а новые, что были добавлены, останутся видимыми. И вот, чтобы они пропали, как и должны пропасть по задумке, нужен этот самый дополнительный этап фильтрации.
          - исправлен код команды «Переснять скриншот» учитывая новую систему фильтрации. Выполненную команду теперь можно отменить через историю действий
          - исправлен код команды «Нарисовать содержимое на фоне и удалить содержимое» переработан так, чтобы её можно было отменить через историю действий
          - фича для отладки, добавленная ранее, теперь называется «Нарезать фон на куски» и её можно вызвать из контекстного меню
          - команда «Нарезать фон на куски» теперь может быть отменена через историю действий
          - исправил баг в команде «Нарезать фон на куски»: смещешение одного из кусков в начало координат
          - замечание: только что понял, что дополнительный этап фильтрации на самом деле особо не нужен, потому что новые фоны я мог добавлять в новый слот, и они бы тусили в этом слоте, и больше никаких проблем, потому что при отрисовке рисуются сначала фоны, а потом поверх них пометки, лол
      - пофиксил сохранение проектных файлов: раньше программа не могла адекватно сохранять картинки
      - при сохранении скриншота в память на нём больше не проставялется дата, даже если установлен флажок «ДатаВремя»
      - debug mode: в режиме дебага при потере фокуса редактором или панелью инструментов будет появлятся сообщение
      - UX: теперь при использовании опции «Сохранить результат в память» иконка в трее показывает число картинок сохранённых в память
      - пометки обновлённого инструмента «Текст» теперь сохраняются в проектных файлах и загружаются из них
      - пофиксил ядрёный баг, который мучал уже давно: нашёл я его с помощью `__import__('traceback').print_stack()` внутри метода `elementsParametersChanged`. Оказывается, колбэки чекбокса вызывались при задании значения этого чекбокса, что как бы естественно, но я забыл это учесть, ведь у других параметров значения меняются без вызова колбэков. В итоге у меня больше не появляется checkbox-модификаций при выборе той или ной пометки
          - проверил эффективность фикса на команде «Разрезать на куски», которая раньше баговала, потому что по умолчанию обрезаемая картинка выделялась, что вызывало настройку параметров в панели и ненужный запуск все тех же колбэков
      - задал деактивацию для некоторых команд в контекстном меню в зависимости от контекста, ведь не всегда их можно вызывать, особенно до задания области захвата. К тому же, пункты меню могут быть активированы случайно при промахе, и в результате приложуха крашнется, поэтому всегда нужна защита от дурака в виде деактивации
      - исправлен краш: приложение крашилось при отпускании клавиш-модификаторов во время переноса, масштабирования и вращения
      - протестировал сохранение и открытие файла проекта, в котором были нарисованы пометки всех типов, значит баги значительно уменьшились и можно выпускать в скором времени новую версию программы
      - теперь при вращении пометки «Текст» буквы не будут колбаситься на 1-2 пикселя в стороны благодаря опции «Pixmap-прокси для пометок типа «Текст»» в контекстном меню. По умолчанию она включена
          - прокси-пиксмапа обновляется при редактировании текста, в конце трансформаций и во время трансформаций, а так же при отрисовке, если она ещё не была задана
      - команда «Нарезать фон на куски» теперь работает с тем фоном, который формируют видимые фоновые картинки, а не с тем фоном, который пришёл от скриншота
      - ширина текстового поля автоматически увеличивается при введении текста, как было и раньше
      - команда «Нарисовать содержимое на фоне и удалить содержимое» доработана так, чтобы не обрезать тот контент, который выходит на квадранты с отрицательными координтами (сверху, слева и сверху слева)
      - область выделения пометки «Текст» теперь обновляется при вводе текста, но это можно будет ещё доработать позже

### Версия 0.95 (дата не определена): UX и новые инструменты
  - вернул возможность использования `Ctrl` в зуме по одной из осей
  - нарисовал иконку для нового разрабатываемого инструмента «Горыныч (Дерево стрелок)»: стрелки могут изогнуто расти одна из другой и быть разной ширины; это первый инструмент, который будет учитывать выделенные элементы (или активный элемент) при нанесении последующего элемента, для того, чтобы стрелки можно было экструдить из любого элемента, а любой элемент в свою очередь может иметь одну из ролей в дереве стрелок
  - очень важно: команда «Сохранить результат в память» переименована в «Сохранить результат в лукошко», соответственно переименована и команда «Разложить картинки из памяти»
  - UX: теперь можно сохранять скриншоты без закрытия редактора - для этого по синей кнопке «Готово» надо нажать правой кнопкой мыши
  - DEBUG: отрисовка списка модификаций улучшена подробностями: отображается больше параметров пометок и отображается позиция индекса истории действий через стрелку у конкретного слота
  - исправлен баг: вернул уведомление о создании скриншота при клике левой кнопкой мыши на кнопку «Готово»
  - напоминаю сам себе, что скриншот скриншотера можно сделать через опацию «Сохранить результат в лукошко». В этом режиме повторный вызов редактора до завершения работы текущего редактора не блокируется
  - UX: поменял отрисовку рамки каждого выделенного элемента: вместо белого выбран цвет Qt.magenta
  - UX: стрелка, сопровождающая текст, теперь обновляет своё начало при редактировании этого самого текста
      - теперь позиция поправляется сразу при нанесении
  - UX: вывод дебажной инфы улучшен: теперь она учитывает зум вьюпорта (влияет на список пометок и список слотов модификаций)
  - ещё один виток по орбите UX: окно редактора скриншота теперь не перекрывает панель задач в Windows и Linux
      - проработано три случая: панель задач снизу (как обычно), панель задач слева и панель задач сверху. Четвёртый случай с расположением справа не учитывался, ибо справа у меня начинается второй монитор и скриншотер простирается и на его пространство тоже
      - теперь больше не придётся пользоваться командой контекстного меню «Свернуть на панель задач» чтобы спрятать редактор, ибо панель задач всегда доступна и можно кликнуть по иконке скриншотера для скрытия и показа
      - для сохранения прежнего опыта холст смещается, чтобы сам скриншот в редакторе появлялся без смещения относительно экрана как и раньше
  - UX: для выделенных пометок теперь доступна обыкновенная операция копирования через сочетания `Ctrl+C` и `Ctrl+V`
  - произвёл рефакторинг: вынес второстепенный код из файла `oxxxy.py`
  - UX: активная пометка теперь отмечается дополнительным пунктиром, который выходит за границы габаритов пометки
  - UX: аккуратная область выделения для пометок инструментов «Линия», «Карандаш», «Маркер», «Стрелка»
      - сделана тестовое формирование области выделения и её отрисовка для инструментов «Карандаш» и «Маркер»
      - задаётся минимальная величина расширения области независимо от заданного размера
      - исправил все методы выделения на работу в аккуратном режиме
      - полностью реализован алгоритм построения области выделения
          - для инструментов «Карандаш» и «Маркер», в том числе когда они выпрямлены (`Element.straing==True`)
          - для инструмента «Линия»
          - для инструмента «Стрелка»
          - для инструмента «Текст», когда наносится вместе со стрелкой
      - область выделения перестраивается, если меняются параметры, а именно размер
  - UX: зависимые от фона пометки теперь обновляются на лету во время процесса трансформации, а не только под конец процесса
  - замечание по UX: программа начинает заметно подтормаживать, когда использование CPU превышает 50% (это происходило во время просмотра видео на Youtube разрешением 1920x1080 и частотой 60fps, процессор intel i5-4670)
  - баг: курсор текстового поля оставался нарисованным в финальных картинках
  - баг: вернулась отрисовка подложки в режиме дебага, которая пропала в результате недавней переделки
  - инструмент «Дерево стрелок»
      - реализовано нанесение пометки
      - техническая отрисовка пометки (эта отрисовка не будет отображаться на финальном результате)
      - деревья стрелок должны отрисовываться после всех остальных пометок, а техническая отрисовка должна лежать поверх деревьев
      - схематическая отрисовка ребёр
      - для того, чтобы ребро было созданно не с предыдущим созданным узлом, а с любым другим, но этот узел надо выделить кликом, а не резиновым прямоугольником
      - подтипы элементов инструмента:
          - корневой: первый узел в дереве, задающий начало
          - связующий: узел между корневым и листовыми или другими связующими узлами
          - листовой: конечный узел, отображается как стрелка
      - первый подэлемент в дереве считается корневым; если добавляется второй, то этот второй считается листовым; если ко второму добавляется третий, то второй считается теперь связующим, а а третий - листовой
      - UX: больше не нужно выделять узел, чтобы новый узел был соединён с выделенным. Теперь достаточно просто начать нанесение нового узла из той позиции, где находится нужный узел. Нужный узел выбирается по принципу близости. Если связи не нужно, то надо зажать клавишу `Ctrl`
      - UX: новые узлы, если соединяются с другими, соответствующим образом поворачиваются в зависимости от положения соседа
      - связующие узлы теперь отрисовываются, отрисовка черновая, необходима доработка
      - корневые узлы теперь отрисовываются, отрисовка черновая
      - листовые узлы теперь отрисовываются, отрисовка черновая
      - поправил отрисовку: техническая отрисовка снова стала просачиваться в финальный результат
  - отмена разработки инструмента «Пипетка»
      - реализация: дополнительное окно размером на все экраны на котором нарисован скриншот, созданный на момент вызова инструмента. Пипетка сэмплирует пиксили на этом окне.
      - сейчас инструмент не реализовать из-за того, что слайдер цвета не поддерживает разную насыщенность цвета
  - инструмент «Дерево стрелок»
      - изложу мысли насчёт операций редактирования и прочего
          - подразделение: потребуется только для случая, когда всё будет отрисовываться кривыми Безье
          - удаление: ненужная операция, тем более непонятно что делать, когда удаляется связующий узел. Да, два ребра конечно пропадут, но зачем удалять? Может лучше не рисовать то, что потом придётся удалить?
          - ввёл подтипы узлов, из-за этого и у связуемого узла во время редактирования дерева пришлось бы создавать модификацию, чтобы после удаления соседнего узла подтип узла возвращался к прежнему. Однако это не нужно, потому что у корневого узла подтип задаётся сразу и его менять не нужно, а связующие и листовые узлы легко отличать по количеству ребёр, соединённых с ними.
      - во время нанесения теперь рисуется линия к ближайшей ноде
  - переработал прототип кастомной отрисовки QTextDocument
      - исправил отрисовку курсора для фрагментов, которые были созданы из-за символа переноса
      - реализовал отрисовку выделения
  - исправлена отрисовка курсора в текстовом поле пометки «Текст» (исправление поправляет отрисовку для всех строк, кроме первой, с первой всё было ОК изначально)
  - теперь в имени файла проекта указывается внутренний формат (cbor2 или json)
  - UX: теперь при использовании режима коллажей задаётся сдвиг панорамирования (viewport panning) таким образом, чтобы первая картинка в коллаже располагалась по центру первого монитора
  - UX: сохранение и загрузка теперь доступны через сочетания `Ctrl+S` и `Ctrl+O`
  - новая команда в контекстном меню для расставления элементов рядами и столбцами (или проще говоря - сеткой)
      - если количество рядов не равно нулю, то на его основе будет вычислено количество столбцов, при этом заданное количество столбцов игнорируется
      - перед расставлением автоматически вызывается команда «Автоколлаж» с опцией «по горизонтали». Она масштабирует все картинки так, чтобы у них у сех была задана одинаковая высота и ориентируется на высоту картинки с наибольшей высотой
      - в конце задаётся область захвата
  - прекращение отрисовки точечного выделения вокруг активного элемента, если он уже невидим
  - рефакторинг имён атрибутов в преддверии интеграциии на страницу Board в Krumassan Image Viewer
      - `Element.type` -> `Element.oxxxy_type`
      - `Element.subtype` -> `Element.oxxxy_subtype`
      - `Element.element_position` -> `Element.position`
      - `Element.element_rotation` -> `Element.rotation`
      - `Element.element_scale_x` -> `Element.scale_x`
      - `Element.element_scale_y` -> `Element.scale_y`
      - `Element.element_width` -> `Element.width`
      - `Element.element.height` -> `Element.height`
  - нужно переносить все наработки из Image Viewer:Boards для инструмента «Текст». Сразу я этого не сделал, поэтому придётся помучаться, но щито поделать
      - восстанавлено влияние масштабирования пометки на размер текста, то есть теперь размер текста задаётся через виджет трансформации, при этом надо удерживать `Shift`, чтобы масштабирование было пропорциональным. Отсюда выходит, что стена текста больше не будет сохранять заданную ширину при вводе этого самого текста
      - перенесено:
        - UX: теперь у пометки будет отступ от рамки виджета (чтобы было видно курсор в самых крайних его расположениях)
        - размер шрифта пометки теперь полностью задаётся через виджет трансформации айтема
        - реализован мигающий курсор ввода текста
        - пометка входит в режим редактирования текста сразу после создания
        - UX: теперь можно задавать прозрачность и цвет для подложки и текста через специальные кнопки сверху слева от пометки
        - UX: выделение текста внутри айтема-заметки (ITEM_NOTE)
          - редактирование текста активируется через двойной клик левой кнопки мыши
          ...
          - поддержка отмены действия и повторения отменённого действия (`Ctrl+Z`, `Ctrl+Y`) в режиме редактирования текста
          - UX: курсор перестаёт мигать во время изменения своей позиции
          ...
          - UX: поддержка клавиш-стрелок `Вверх` и `Вниз` для выделений текста и переноса курсора между строками
          - UX: drag'n'drop внутри текстового поля для выделенного текста через зажатую левую кнопку мыши
              ...
              - при активации drag'n'drop задаются курсоры в зависимости от того зажата ли клавиша `Ctrl`
              - отмена через нажатие клавиши `Esc` в процессе drag'n'drop
              - активируется блокировка взаимодействия с клавиатурой до отпускания левой кнопки мыши
  - код перенесён, осталось лишь доработать UX и убрать некоторые баги
  - UX: сделал временное решение для того, чтобы при назначении активной пометкой, эта пометка ещё дополнительно выделялась
  - инструмент «Текст»: задание размера через панель инструментов отключена. Также отключено задание размеров и подложки (у подложки можно выкрутить полную прозрачность)
  - (12 май 24) исправил все косяки, которые нашёл прямо сейчас. Другие косяки и краши, если они найдутся, будут исправлены в дальнейшем. Пока можно считать, что всё вроде бы ОК
    - уменьшил начальный размер шрифта для пометки «Текст»
    - отключил прокси-пиксмапу. Отчего-то она не обновляется при масштабировании. И отчего-то без неё больше нет однопиксельных скачков текста. Разбираться с этим времени пока нет
    - пришлось подумать, чтобы сегодняшнее UX-решение насчёт выделения новой активной пометки полностью заработало и для пометки «Текст»
  - (18 май 24) команда взаимного обмена индексов сортировки у картинок (для команды «Выложить сеткой»)
    - отрисовка индекса `Element.unique_index`
    - отрисовкой индекса можно управлять через клавишу `I`
    - через сочетание `Ctrl+I` можно менять индексы. Это задумано для двух выделенных картинок, но можно и больше. В таком случае индексы будут заданы со сдвигом, например: 1->3, 2->1, 3->2
    - важно: смена индексов не прописывается в истории действий!
    - теперь перед командой «Выложить сеткой» автоматически отменяется выделение
    - теперь в команде «Выложить сеткой» порядок картинок определяется не по X координате, а по индексу сортировки
    - `Element.unique_index` заменён на `Element.sorting_index`, потому что `unique_index` используется в системе истории
    - чтобы выделить картинки выборочно по одной надо зажимать `Ctrl` при клике
  - (24 май 24) команда «Задать область захвата по содержимому» больше не задаёт отступ для картинок
  - (3 июл 24) исправлен краш: при мультикадрировании легко создать рамку нулевой ширины и длины, и собственно, затем такая рамка крашит создание финального изображения
    - решение: удаление такой пометки через отмену в истории действий
  - (14 авг 24) исправлен баг: редактор коллажей не учитывал метаданые изображения об ориентации

### - (15 фев 25) назрел пересмотр всего UX, чтобы и бесячие баги исправить, и окончательно сплавить нарощенный функционал с основным. Весной этим и займёмся
  - (14 апр 25) исправил бесячий баг инструмента «Затемнение»
    - в финальной версии с затемнение работало как надо
    - но в редакторе внешняя граница затёмнённой области не реагировала на паннинг и зум
  - (18 апр 25) команда «Автоколлаж» теперь сохраняет отзеркаливание изображений по двум осям
  - (21 июл 25) баг: образ кисти у инструмента «Маркер» при перемещении курсора мыши пропадает в некоторых местах над областью рисования, а должен всегда отображаться в этой области (неправильная конвертация координат)
  - (14 сен 25) лейбл разрешения захвата теперь выводится через на чёрную подложку, чтобы разрешение было видно и на светлых сриншотах
  - (25 сен 25) новый пункт в контекстном меню *Режим создания обоины для рабочего стола*
    - два варианта для итогового разрешения: 2560x1440 и 1920x1080. Последнее разрешение можно получить, если кликнуть ещё раз по пункту меню
  ...
  - (4 окт 25) при многократном вызове скриншотера (от 60 до 80 раз подряд) он в итоге крашится, и как следствие, его приходится запускать вручную.
    - характерные явления
      ...
      - (12 фев 26) попытка решить эту проблему так - уведомления теперь запускаются по параллельному файлу запуска. Посмотрю, поможет или нет
  - (16 дек 25) UX: при занесении результата в буфер обмена копируется не только картинка, но и путь к ней. В результате там, где нужен текст, будет скопирован текст, а там где картинка - только картинка
  - (1 янв 26) UX: НЕ РЕШЕНО при запуске DirectX-приложений в полноэкранном режиме начинается макарена
      - после выхода из такого приложения, скриншотер думает, что разрешение монитора не вернулось обратно, и в итоге редактор скриншота отображается с очень маленькой высотой окна. Однако, если при такой макарене всё же создать скриншот или отменить его создание, то приложение перезапускается, и всё начинает работать как раньше, без макарены. Дело всё в том, что availableGeometry выдаёт неправильные значения. У меня подозрение, что сообщение об обратном изменении разрешения мониторов просто не проходит или даже не генерируется, отсюда и проблемы.
      - ранее я пытался записывать возвращаемое значение availableGeometry сразу после старта, и это решил проблему с окном, но менюхи при вызове появлялись со сдвигом. Также на скриншоте появлялся чёрный квадрат, закрывающий содержимое первого монитора. Всё это очень странно
      - думаю, может запретить вообще обработку сообщений об изменении разрешения или, что может быть лучше даже - при вызове редактора заставлять программу отправлять самой себе сообщение об изменении разрешения и запрашивать это разрешение у API, только я пока не знаю как это сделать
  - (19 янв 26) оказывается, при вызове редактора из окна уведомлений (оно отображается после создания скриношота), этот самый редактор открывался в дебажном режиме
      - 2 дня назад я костыльно сделал фикс этой проблемы, но сейчас исправил и этот косяк
  - (1 фев 26) добавил несколько вариантов для курсора на скриншоте
      - можно принудительно выбрать курсор из предложенных вариантов в контекстно меню через пункт «Выбрать курсор для скриншота...»
      - программа пытается выяснить, какой тип курсора выставлен перед нажатием клавиши создания скриншота. Если это курсор PointingHand, то он будет автоматически выбран при вызове скриншотера, и не придётся лезть в контекстное меню лишний раз
  - (11-13 фев 26) новое завоевание в сфере UX, отсутствие которое долго мазолило вообще всё
      - описание проблемы и её решения
          - !!! раньше для редактора коллажа можно было выбрать несколько картинок и, перетащив их из проводника в окно у трея, автоматически зайти в этот самый редактор, а в редакторе уже были разложены картинки, которые перетащили на окно. Однако, перетащить можно было только один раз, то есть, дотащить что-то из другой папки, и зайти в редактор коллажей после того, как всё притащил, было невозможно. Но теперь это стало возможным.
      - под эту фичу заведена специальная дебаг-переменная `DEBUG_INPUT_FILES_TRAY_WINDOW`
      - подготовлено специальное вспоомогательные окошко со списком картинок, оно откроется как только хотя бы один файл будет скинут на основое окошко программы
          - если какая-то картинка не нужна, можно её удалить из списка, нажав на крестик сбоку
          - порядок картинок в списке можно менять перетаскиванием
          - как только все будут в сборе, можно нажать кнопку «В редактор»
          - можно отчистить список и поменять их порядок на обратный через кнопки внизу
          - если будет перекидываться папка из проводника, то программа будет в ней искать картинки и загружать их превью. И если нужно просканировать и подпапки, то нужно держать отмеченным соответствующий чекбокс внизу списка
          - также в этом списке можно выбрать папку для сохранения результата. Щёлкая по превьюшкам в списке можно выбрать папку - то есть, будет выбрана папка, из которой было загружено то или иное изображение. Если нужна папка по-умолчанию, то можно включить соответствующий чекбокс внизу
      - как только будут закинуты картинки из проводника - будет запущен поток для их загрузки
      - понятно дело, что поддерживаются только картинки
      - для тестированяи и отладки
          - вспомогательное окно надо закрывать перед тем как входим в редактор
          - если кнопка «В редактор!» нажата до того, как успелись сгенериться превьюшки и загрузится картинки, то программа заставит вас её подождать, пока этот процесс закончится
      - (16 фев 26) перенёс кнопку «В редактор» в шапку окна, там удобней по ней нажимать
      - (16 фев 26) после нажатия на кнопку «В редактор» на скриншоте пропечатывалось вспомогательное окно. Теперь так не не происходит
  - (16 фев 26) в контекстное меню добавлено больше иконок, удля удобства навигации
  - (16 фев 26) у кнопки «Готово» появился близнец снизу, она будет сохранять скриншот только в буфер, минуя сохранение в файл
      - в результате нажатия на неё появится специальная версия уведомления, которая позволит только открыть результат в Oxxxy 
      - фича остро нужна в ситуациях, когда скриншот делается только для того, чтобы запульнуть в мессенджер, и не хочется затем удалять его из папки сохранения
  - (16 фев 26) снаппинг ко краям мониторов при первоначальном задании рамки захвата и её последующей правке
      - !! теперь началась благодать. В основном это нужно, чтобы рамка снаппилась к границе между мониторами, в итоге это даёт возможность не обрезать скриншот и не заниматься точной подстройкой рамки захвата с помощью увеличения масштаба отображения холста
      - !!! (24 фев 26) снаппинг, оказывается, работает и при разных значениях зума. Я не планировал такое при разработке, но в итоге всё работает. Классно!
          - !!! (27 фев 26) оказывается, снаппинг работал при разных значениях зума только при изменении рамки захвата, а при первоначальном нет, но теперь я исправил и этот косяк
  - (16 фев 26) изменён внешний вид чекбоксов в настройках 
  - (18 фев 26) изменён визуальный стиль чекбоксов, теперь они выглядят как переключатели
      - изменение касается контекстного меню и панели инструментов
  - (18 фев 26) чекбоксы в контекстном меню прокликивались сквозь, если клик производится в их правой части свободной от текста
      - я очень долго возился с css-стилями и лейаутами, но так и не смог одолеть проблему
        - в итоге я решил вычислять длину строки самого длинного пункта, а остальные пункты дополнять символами пробелов, чтобы текстовая часть удлинилась, и в итоге это сработало
          - однако, проблема осталась в пространстве между пунктами меню  
  - (20 фев 26) дополнил и починил инструмент мультикадрирования
      - раньше, если больше 1-2 раз нажать клавишу `P`, то результирующее изображение пугающе преображалось с каждым последующим нажатием
      - теперь можно выкладывать картинки не только вертикально, но и горизонтально. Для смены одного на другое достаточно ещё раз активировать инструмент
  - (20 фев 26) **приём для работы: когда нужно сделать коллаж одного объекта из серии картинок, то можно сначала сделать автоколлаж, чтобы собрать их вместе, забейкать их в фон, а потом воспользоваться мультикадрированием, чтобы избавиться от пустых мест на всех картинках, выделив только сам объект**
  - (23 фев 26) форму редактора коллажей теперь можно вызвать через хоткей `Alt+Print Screen`. Если активирована русская раскладка, то правый Alt перестаёт срабатывать, и нужно зажимать левый
      - фича крайне нужная, так как за неделю с лишним надоело вызывать окно в трее, целясь в иконку в трее и перетаскивать файлы на форму, лучше чтобы она сразу появлялась по хоткею
  - (23 фев 26) иконка в трее теперь либо чёрная, либо белая в зависимости от выбранной темы Windows
  - (23 фев 26) редактор коллажей теперь поддерживает гифки
      - гифки могут содержать много кадров, и они все будут разложены на холсте, не будет варианта выбрать подходящие - придётся удалять ненужные уже на холсте 
      - паннинг и зум в помощь
  - (23 фев 26) при нажатии правой кнопки мыши на кнопку «Готово» теперь вылезает сообщение о том, что начался процесс
  - (24 фев 26) **в списке элементов к каждому добавляется время его создания, для удобства отладки/исправления багов**
  - (24 фев 26) UX: окно уведомлений о создании скриншота больше не ворует себе фокус ввода
  - (27 фев 26) ввёл режим Burst mode (continuous shooting mode)
      - надо зажать сочетание клавиш `Ctrl+Alt+Print`, и пока они удерживаются, будет идти непрерывная съёмка экрана, в котором находится курсор мыши
      - в трее будет отображаться, сколько кадров уже нащёлкано
      - после того, как клавиши будут отпущены, откроется редактор коллажей, в котором все снимки будут вертикально расположены друг за другом
          - для захвата нужного содержимого с них, нужно забейкать их в фон, и потом применить инструмент мультикадрирования
  - (11 мар 26) теперь на форму файлов редактора коллажей можно перетаскивать картинки из браузера - oxxxy их должен загрузить и положить в список файлов
      - надо учитывать, что если на браузере установлен браузерный VPN и картинка доступна только через VPN, то oxxxy не сможет её скачать. Для избавления от этой проблемы нужно устаналивать VPN на весь комп, а не только на браузер
  - (13 мар 26) теперь можно быстро задать область захвата площадью с выбранный монитор
      - выбор производится положением курсора мыши
      - задание работает через клавишу `F9` 
  - (11 май 26) надоело, что прямоугольники захвата у инструмента мультикадрирования пропадают, когда я вызываю команду «Нарисовать содержимое на фоне»
      - исправил источник этого раздражения
