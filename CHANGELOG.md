...
## Что планируется сделать дальше?
  - **Основное**
      ...
      - реализовать намного более точное выделение объектов для инструмента **Выделение и трансформации**, а не по bounding box объекта как сейчас
...
### Версия 0.94 (дата не определена)
  ...
  - внедрение PZMT (Canvas Panning & Zooming и Elements Multiselect & Transformations) для холста в редакторе
      ...
      - зум холста (zomming) работает через колесо, **пока колесом нельзя менять параметры инструментов как раньше, метод change_tools_params не вызывается**
  ...
  - внедрение PZMT (Canvas Panning & Zooming и Elements Multiselect & Transformations) для холста в редакторе (продолжение)
      ...
      - восстановлена работа инструмента «Картинка» с поддержкой нового выделения, собственных трансформаций и pan & zoom
      ...
      - восстановление работы инструменов «Лупа» и «Копипейст»
          - для начала сделал так, чтобы не крашилось при нанесении и отрисовке, баги не исправляются, ибо их присутствие сейчас это ОК
      ...
      - Лупа: отрисовка соединительных линий без транформации, учитывая поворот обоих элементов
          - точки первого виджета надо получить относительно канваса и уже повёрнутые и отскейленые
      ...
      - исправление краша: после Delete нужно снимать выделение с удалённой картинки. Вдобавок надо было прописать реакцию на пометки типа ToolID.removing для кода выделения и создания виджета трансформаций
      ...
      - инструмент «Мультикадриоование» теперь поддерживает фрагменты с вращением
      - команда «Автоколлаж» налажена и теперь поддерживает трансформированные картинки
          ...
          - «Подогнать все картинки по размеру под одну» подгоняет остальыне картинки под активную картинку (активная картинка это выделенная кликом картинка). Если активной картинки не будет, то результат будет схожим с командой «Автоколлаж»
