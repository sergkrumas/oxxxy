...
### Версия 0.95 (дата не определена): UX и новые инструменты
  ...
  - ещё один виток по орбите UX: окно редактора скриншота теперь не перекрывает панель задач Windows
      ...
      - для Linux решения нет. Потому что на Ubuntu панель задач находится справа, но решения для Linux пока не разработано
  ...
  OXXXY
      - (22 мар 24) UX: выделение вытянутых объектов через аккуратную область выделения: доделать
            - отрисовка у всех пометок в режиме дебага
            - функция получения области для операций выделения во вьюпорте
            - замена функции для выделения
            - функция получения selection_path в зависимости от типа объекта и состояния переменных
            - нужна минимальная ширина
                - Карандаш
                    путь и local_point
                - Маркер
                    путь и local_point
                ...
                - Стрелка
                - Стрелка с текстом
            - функцию надо вызывать и при изменении размеров слайдером
