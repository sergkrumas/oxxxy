...
## Что планируется сделать дальше?
  - **Основное на 2024**
      - пересматривать и перерабатывать UX и UI в свете того, что Oxxxy отныне может редактирвоать коллажи, а не просто делать скриншоты с пометками
      - (25 мая 23) инструмент «Направляющая» (линейка для прямых линий, круглый транспортир для дуг окружности) как в скриншотере Windows
      - (25 мая 23) инструмент «Овал» должен поддерживать дуги для окружности и эллипса (при нанесении и при редактировании)
      - (18 мар 24) текст по кривой линии и по окружности, прочие деформации типа этих https://doc.qt.io/qt-6/qtwidgets-painting-deform-example.html
      - (25 июл 23) UX: автоколлаж: паковка картинок под определённые ширину и высоту или под их соотношение: фичу надо сначала реализовать для BOARDS, а потом взять код оттуда, смотреть в сторону алгоритмов UV packing из 3D-редакторов
      - (21 май 24) UX: интерактивное создание коллажей
          - картинки можно менять местами перетаскивая одну на другую, и после отпускания кнопки мыши алгоритм расстановки обновляет холст
      - перенести код с PyQt5 на PyQt6
      - капитально отрефакторить код
      - добавить возможность менять размеры шрифтов в UI
      - добавить возможность записывать видео со звуком или гифки в захваченной области (фон в таком случае не рисуется вообще, работает только виджет захвата, возможно
      потребуется создание расширения языке Си)
        - (aka liceCAP - simple animated screen captures) - https://www.cockos.com/licecap/
        - git clone https://www-dev.cockos.com/licecap/licecap.git
        - Official GitHub mirror: github.com/justinfrankel/licecap
      - (30 окт 23) рассмотреть программу ShareX и может чего взять оттуда https://getsharex.com/
      - UX: несколько идей по поводу цикла жизни процесса приложения
          - идея 1: после запуска программа сидит в трее и ждёт нажатия горячих клавиш. При нажатии запускает свою копию с другими параметрами, которые делают скриншот и запускают редактор для него
          - идея 2: использовать нынешнюю систему и запускать новый инстанс скриншотера как только нажата клавиша. Новый инстанс будет слушать нажатия клавиш, а действующий инстанс позволит редактировать скриншот, при этом действующий инстанс обрабатывать нажатия клавиш уже не будет, у него на это будет стоять блокировка
  - **Подготовка к релизу**
      - перевести UI на английский язык
...
## Известные проблемы и юзабилити
- сейчас панель заезжает на второй экран, что неудобно, пусть лучше она останется на первом экране и будет показываться сверху или снизу или внутри
    - хотя ведь можно просто переместить да и всё или нажать `Enter`

- при открытии менюшки со стикерами вылезает такое: QObject::~QObject: Timers cannot be stopped from another thread https://forum.qt.io/topic/53569/solved-timers-cannot-be-stopped-from-a-different-thread/4
...
### Версия 0.94 (19 марта 2024): панорамирование и зум для редактора; возможность выделения нескольких пометок; перемещение, вращение и масштабирование пометок; обновлённая система истории действий
  ...
  - внедрение PZMT (Canvas Panning & Zooming и Elements Multiselect & Transformations) для холста в редакторе (продолжение)
      ...
      - инструмент «Текст»
          - при трансформациях текста начало сопроводительной стрелки перемещается так, чтобы быть ближе к любой точке на периметре геометрической оболочки текста, это относится к трансформациям самой стрелки. Так было и раньше, и в оригинальном Joxi, так что я просто возвращаю функнционал
      ...
      - при удалении группы (клавиша `Delete`) пометок теперь нужен всего одна невидимая removing-пометка
      ...
      - рефакторинг метода создания и обновления фона скриншота
          ...
          - команда «Нарисовать содержимое на фоне» больше не отменяет историю действий, но ещё пока не работает с ней так подобает
...
### Версия 0.95 (дата не определена): UX и новые инструменты
  ...
  - UX: теперь при использовании режима коллажей задаётся сдвиг панорамирования таким образом, чтобы первая картинка в коллаже располагалась по центру первого монитора
