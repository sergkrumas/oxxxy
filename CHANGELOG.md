...
### Версия 0.95 (дата не определена): UX и новые инструменты
  ...
  - нужно переносить все наработки из Image Viewer:Boards для инструмента «Текст». Сразу я этого не сделал, поэтому сейчас придётся мучаться, но щито поделать
      - восстанавлено искажение из-за масштабирования. Теперь размер текста задаётся через виджет масштабирования, при этом лучше удерживать Shift, чтобы масштабирование было пропорциональным
