...
class CustomSlider(QWidget):
    ...
    def __init__(self, _type, width, default_value, flat_editor_ui):
        super().__init__()
        ...
        self.image = None
        self.raw_value = 1.0
        self.flat_editor_ui = flat_editor_ui
        self.refresh_image()
    ...
    def paintEvent(self, event):
        ...
        if self.isEnabled():
            if self.type == "SCALAR":
                ...
                painter.setClipping(False)
            if not self.flat_editor_ui or self.type != "SCALAR":
                # draw button
                path = QPainterPath()
